...
func (s *Server) handleSearchThreads(req rpc.Request) rpc.Response {
	...
	page, err := rt.client.SearchThreads(ctx, params.Query, int64(normalizeSearchLimit(params.Limit, s.cfg.MaxSearchResults)), params.PageToken)
	if err != nil {
		return mapGmailError(req.ID, err)
	}

	threads := make([]rpc.ThreadSummary, 0, len(page.Threads))
	for _, item := range page.Threads {
		if item == nil || strings.TrimSpace(item.Id) == "" {
			continue
		}
		thread, err := rt.client.GetThreadMetadata(ctx, item.Id)
		if err != nil {
			return mapGmailError(req.ID, err)
		}
...
func (s *Server) handleSearchMessages(req rpc.Request) rpc.Response {
	...
	page, err := rt.client.SearchMessages(ctx, params.Query, int64(normalizeSearchLimit(params.Limit, s.cfg.MaxSearchResults)), params.PageToken)
	if err != nil {
		return mapGmailError(req.ID, err)
	}

	if params.IncludeBody {
		messages := make([]rpc.MessageDetail, 0, len(page.Messages))
		for _, item := range page.Messages {
			if item == nil || strings.TrimSpace(item.Id) == "" {
				continue
			}
			meta, err := rt.client.GetMessageMetadata(ctx, item.Id)
			if err != nil {
				return mapGmailError(req.ID, err)
			}
			if !rt.allowsMessage(meta) {
			...
			full, err := rt.client.GetMessageFull(ctx, item.Id)
			if err != nil {
				return mapGmailError(req.ID, err)
			}
	...
	messages := make([]rpc.MessageSummary, 0, len(page.Messages))
	for _, item := range page.Messages {
		if item == nil || strings.TrimSpace(item.Id) == "" {
			continue
		}
